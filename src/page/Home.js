...
const Home = () => {
    ...
    const alluser = async (ids) => {
        try {
            axios.get(`http://mgt2.pnu.ac.th/fizon/6460704001/users.php`)
                .then(res => {
    ...
    const deleteUser = async (id) => {
        try {
            axios.post(`http://mgt2.pnu.ac.th/fizon/6460704001/deleteusers.php`, {
                userids: id,
            })
                .then(res => {
                    setuser([]);
        ...
        <div>
            ...
            {isuser.map((item, index) => (
                <div className="list" key={item.user_id}>
                    ...
                    <p>Date: {item.date}</p>
                    <Link to={`edit/${item.user_id}`} className="btn default-btn">
                        Edit </Link>
